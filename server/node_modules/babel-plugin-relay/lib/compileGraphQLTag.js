...
function createNode(t, state, path, graphqlDefinition, options) {
  ...
  var id = topScope.generateDeclaredUidIdentifier(definitionName);
  ...
  var expAssignAndCheck = t.SequenceExpression([expAssignProd, expWarnIfOutdated, id]);
  var expAssign;

  if (options.isDevVariable != null) {
    expAssign = t.ConditionalExpression(t.Identifier(options.isDevVariable), expAssignAndCheck, expAssignProd);
  ...
  path.replaceWith(t.ConditionalExpression(t.BinaryExpression('!==', id, expVoid0), id, expAssign));
}
